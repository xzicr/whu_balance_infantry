...
fp32 jump_stand_PD_L[2] = {100000000.0f,2000.0f};
fp32 jump_stand_PD_R[2] = {100000000.0f,2000.0f};
...
fp32 IDEAL_PREPARING_STAND_JUMPING_ANGLE = 0.174532f;
    ...
    else if (chassis_mode_change->mode.jumping_mode == STANDING_JUMP)
    {
	 // 站立跳跃状态机
        switch(chassis_mode_change->mode.jumping_stage)
        {
            ...
            case EXTENDING_LEGS:
                // 腿伸长阶段
                if (chassis_mode_change->chassis_posture_info.leg_length_L >= 0.36f && 
                    chassis_mode_change->chassis_posture_info.leg_length_R >= 0.36f)
                {
                    chassis_mode_change->mode.jumping_stage = CONSTACTING_LEGS_2;
            ...
            case CONSTACTING_LEGS_2:
                // 再次收缩腿准备落地
                if (chassis_mode_change->chassis_posture_info.leg_length_L <= 0.13f)
                {
                    chassis_mode_change->mode.jumping_stage = PREPARING_LANDING;
                }
            ...
            case FINISHED:
                // 跳跃完成，重置状态
				// chassis_mode_change->chassis_posture_info.foot_distance_set += 2.0f; //明天可以试一下就是每次完成一次跳跃设定值就加一点来保证往前走
                chassis_mode_change->mode.jumping_stage = READY_TO_JUMP;
                chassis_mode_change->mode.sport_mode = NORMAL_MOVING_MODE;
...
void Target_Value_Set(chassis_move_t *target_value_set)
{
	...
	//位置环控制器
	if(fabs(target_value_set->chassis_posture_info.foot_speed_set)!=0)//||target_value_set->mode.sport_mode==JUMPING_MODE)
	{
...
void Chassis_Torque_Calculation(chassis_move_t *bl_ctrl)
{
	...
	{
		//新增只有在非跳跃模式下才能进行离地的弱PID控制
		if(bl_ctrl->mode.sport_mode != JUMPING_MODE)
		{
...
//	FZX的改版
	if (bl_ctrl->flag_info.suspend_flag_R == 1)
	{
		...
		bl_ctrl->torque_info.foot_moving_torque_R = 0;
			// -suspend_foot_speed_Kp * (0.0f - bl_ctrl->foot_motor_R.speed_kf);         1.10可以试试看
	}
	if (bl_ctrl->flag_info.suspend_flag_L == 1)
	{
		bl_ctrl->torque_info.joint_balancing_torque_L = (
...
uint8_t leg_length_ready,leg_angle_ready,leg_gyro_stable,pitch_stable,both_feet_on_ground ,speed_zero,prepare_complete;
uint8_t Check_Jump_Preparation_Complete(chassis_move_t *chassis)
{
