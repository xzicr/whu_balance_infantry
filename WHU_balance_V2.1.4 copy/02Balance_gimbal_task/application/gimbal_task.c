...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
  ...
  // 云台数据更新
  feedback_update->gimbal_pitch_motor.relative_angle = feedback_update->gimbal_pitch_motor.gimbal_motor_measure->last_encoder;
...
void rc_control(gimbal_control_t *gimbal_control_set,chassis_data_t *chassis_data)
{
  ...
  fp32 vx_set_channel, vy_set_channel,yaw_channel;
  fp32 high_control;
  ...
  chassis_data->high_set = fp32_constrain(chassis_data->high_set,0.0,0.33);
...
void key_control(gimbal_control_t *gimbal_control_set,chassis_data_t *chassis_data)
{
  ...
  if(gimbal_control_set->keyboard & KEY_PRESSED_OFFSET_Q&&gimbal_control_set->keyboard & KEY_PRESSED_OFFSET_SHIFT)  
  {
    ...
    fp32_constrain(chassis_data->high_set,0.1,0.55);
  }
