...
void Leg_dlength_Kalman_Init(chassis_move_t *chassis)
{
    if (chassis == NULL) return;
    // 设置初始状态矩阵
    static float P_Init[4] = {KF_L_ESTIMATE_ERROR, 0, 0, KF_L_ESTIMATE_ERROR};
    static float F_Init[4] = {1, CHASSIS_CONTROL_TIME, 0, 1}; // 状态转移矩阵
    static float Q_Init[4] = {KF_L_PROCESS_NOISE, 0, 0, KF_L_PROCESS_NOISE};
    static float H_Init[2] = {1, 0}; // 观测矩阵 [1,0] 表示只观测速度
    static float R_Init[1] = {KF_L_MEASURE_NOISE};
    ...
    memcpy(leg_kf_L.kf.H_data, H_Init, sizeof(H_Init));
    memcpy(leg_kf_L.kf.R_data, R_Init, sizeof(R_Init));

    leg_kf_L.kf.B_data[0] = CHASSIS_CONTROL_TIME * CONTROL_GAIN;
    leg_kf_L.kf.B_data[1] = CONTROL_GAIN;
    leg_kf_L.filtered_dlength = 0;

    

    Kalman_Filter_Init(&leg_kf_R.kf, 2, 1, 1);
    leg_kf_R.kf.UseAutoAdjustment = 0;
    memcpy(leg_kf_R.kf.P_data, P_Init, sizeof(P_Init));
    memcpy(leg_kf_R.kf.F_data, F_Init, sizeof(F_Init));
    memcpy(leg_kf_R.kf.Q_data, Q_Init, sizeof(Q_Init));
    memcpy(leg_kf_R.kf.H_data, H_Init, sizeof(H_Init));
    memcpy(leg_kf_R.kf.R_data, R_Init, sizeof(R_Init));
    leg_kf_R.kf.B_data[0] = CHASSIS_CONTROL_TIME * CONTROL_GAIN;
    leg_kf_R.kf.B_data[1] = CONTROL_GAIN;
    leg_kf_R.filtered_dlength = 0;


}

// 轮毂电机速度卡尔曼滤波更新函数
void Leg_Kalman_Update(chassis_move_t *chassis)
{
    if (chassis == NULL) return;
    ...
    if (filtered_state != NULL) {
        leg_kf_L.filtered_dlength = filtered_state[0]; // 滤波后的速度
    }
    ...
    if (filtered_state != NULL) {
        leg_kf_R.filtered_dlength = filtered_state[0]; // 滤波后的速度
    }
    chassis->chassis_posture_info.leg_dlength_R_kf = leg_kf_R.filtered_dlength;
}
...
void Record_Leg_Control(chassis_move_t *chassis)
{
    ...
    // 记录当前控制量，用于下一次卡尔曼滤波更新
    chassis->torque_info.last_control_torque_L = chassis->torque_info.joint_stand_torque_L;
    chassis->torque_info.last_control_torque_R = chassis->torque_info.joint_stand_torque_R;
}
