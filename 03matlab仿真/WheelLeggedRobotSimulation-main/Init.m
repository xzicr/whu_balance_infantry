...
% 运行LQR with VMC模块之前请运行此代码块
    ...
    for step=1:length(L0s)
        ...
        Iw=0.5 * mw * R^2; 
        Ip=0; 
        h = 0.11;   % height of the body (m)
        w = 0.260;   % width of the body (m)
        Im=(1/12) * M * (h^2 + w^2);
        g=9.8;
...
%运行LQR with MRAC之前请运行此代码块
  ...
  % 运行MPC模块之前请运行此代码块和下一个代码块
        ...
        clear all;
        % Example parameters
        mb = 6.0;  % mass of the float-based body (kg)
        mw = 1.2;  % mass of the wheel (kg)
        rw = 0.06;  % radius of the wheel (m)
        h = 0.11;   % height of the body (m)
        w = 0.260;   % width of the body (m)
        l = 0.314;   % length of the link (m)
        g = 9.81;  % gravitational acceleration (m/s²)
